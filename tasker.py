...
'''
def input_sting_parse(input_string):
# TODO TODO TODO write the function
    if input_starts_with_tasker:
        pass            
    else:
        print('Error!')

def input_starts_with_tasker(input_string):
    """
    >>> input_starts_with_tasker('gogakal')
    False

    >>> input_starts_with_tasker('tasker')
    False

    >>> input_starts_with_tasker('  tasker    ')
    True

    >>> input_starts_with_tasker('  taskerf    ')
    False

    """
    # TODO rewrite regex under this line using re.VERBOSE
    check = re.compile('''^(\s*)(tasker)(\s+)''')
    if check.match(input_string) is not None:
        return True
    else:
        return False

def slice_off_command(input_string):
# TODO write the function
    slicing_regex = re.compile('''()()''')
'''
